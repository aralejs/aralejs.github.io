define(function(require, exports, module) {
    ...
    var Switchable = module.exports = Widget.extend({
        ...
        _parseRole: function() {
            ...
            if (role) {
                ...
                if (!triggers.length && (role.trigger || role.nav)) {
                    this.set('triggers', parseRole(role.trigger,
                                role.nav, $element), 'SILENT');
                }

                if (!panels.length && (role.panel || role.content)) {
                    this.set('panels', parseRole(role.panel,
                                role.content, $element), 'SILENT');
                }
    ...
    function parseRole(children, parentClass, root) {
        var elems = [];
    ...
    Switchable.autoRender = function(config) {
        ...
        // 属性优先, role其次
        if (role) {
            if (!config.triggers && (role.trigger || role.nav)) {
                config.triggers = parseRole(role.trigger, role.nav, $element);
            }

            if (!config.panels && (role.panel || role.content)) {
                config.panels = parseRole(role.panel, role.content, $element);
            }
            delete dataset.role;
        }
