define(function(require, exports, module) {
    ...
    var CalendarModel = Base.extend({
        ...
        initialize: function(config) {
            ...
            for (var key in config.i18n) {
                ...
                i18n[key].value = translate(config.i18n[key]);
            }
        ...
        renderData: function() {
            this.set('year', activeTime.year());
            this.set('month', activeTime.month());
            this.set('date', activeTime.clone());
            this.set('day');
            this.set('time');
        },

        changeTime: function(key, number) {
            activeTime.add(key, number);
            this.renderData();
        },
        ...
        changeMode: function(mode, obj) {
            obj || (obj = {});
            if ('month' in obj) {
                activeTime.month(obj.month);
            }
            if (obj.year) activeTime.year(obj.year);
            this.set('mode', mode);
            this.renderData();
        },
        ...
        selectDate: function(time) {
            if (time) {
                activeTime = moment(time);
                this.renderData();
            }
            return activeTime.clone();
        },
    ...
    function createMonthModel(month) {
        ...
        current = {
            value: month,
            label: translate(showMonths[month])
        };
    ...
    function createDayModel() {
        ...
        for (var i = startDay; i < 7; i++) {
            items.push({label: translate(DAY_LABELS[i]), value: i});
        }
