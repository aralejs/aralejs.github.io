define(function(require, exports, module) {
    ...
    // Dialog
    // -------
    // Dialog 是通用对话框组件，提供确定、取消、关闭、标题设置、内容区域自定义和动画显隐功能。
    // 是所有对话框类型组件的基类。
    ...
    var Dialog = Overlay.extend({
        ...
        show: function() {
            return Dialog.superclass.show.call(this);
        },

        hide: function() {
            return Dialog.superclass.hide.call(this);            
        },
        ...
        _onChangeContent: function(val) {
            console.log(this.get('contentElement'), this.get('contentElement').html());
            this.get('contentElement').html(val);
            console.log(this.get('contentElement'), this.get('contentElement').html());            
        },

        _onChangeAjaxUrl: function(val) {
            var that = this;
        ...
        _onChangeIframeUrl: function(val) {
            this.$('iframe').eq(0).src = val;
        }
