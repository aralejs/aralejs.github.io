...
(function(){
     ...
     */
    arale.localstorage.getLocalStorage = function(){
     ...
     */
    arale.localstorage.getUserData = function(){
     ...
     * @type {Boolean}
     */
    arale.localstorage.supportLocalStorage = localStorage.isSupport;

    /**
     * 是否支持M$的userData
     * @type {Boolean}
     */
    arale.localstorage.supportUserData = userData.isSupport;

    /**
     * 是否支持本地存储
     * @type {Boolean}
     */
    arale.localstorage.isSupport = localStorage.isSupport || userData.isSupport;

    //初始化arale.localstorage
    if(userData.isSupport){

        arale.mixin(arale.localstorage,userData);

        $E.domReady(function(){
            if(!arale.localstorage.inited) arale.localstorage.init();
        });
    }else if(localStorage.isSupport){
        arale.mixin(arale.localstorage,localStorage);
        arale.localstorage.init();
    }
})();
