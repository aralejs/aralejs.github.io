define(function(require, exports, module) {
    ...
    var Switchable = module.exports = Widget.extend({
        ...
        _switchTo: function(toIndex, fromIndex, direction) {
            ...
            this._switchView(panelInfo, direction);
            return this; // chain
        },
        //切换操作，对外 api
   ...
   //如果navClass传入，但是triggers为空，那么我们自动生成 triggers 的 dom结构。
    ...
    Switchable.autoRender = function(element, elementData, dataset) {
        var role = dataset.role;
        var widgetConfig = dataset.widgetConfig;
        var triggers = [];
        var panels = [];
        var $element = $(element);

        if (role) {
            if (role.trigger || role.nav) {
                if (role.trigger) {
                    var triggerClass = role.trigger.split(',');
                    $.each(triggerClass, function(index, cls) {
                        triggers.push($element.find(cls)[0]);
                    });
                } else {
                    triggers = $element.find(role.nav).children();
                }
            }

            if (role.panel || role.content) {
                if (role.panel) {
                    var panelClass = role.panel.split(',');
                    $.each(panelClass, function(index, cls) {
                        panels.push($element.find(cls)[0]);
                    });
                } else {
                    panels = $element.find(role.content).children();
                }
            }
            delete dataset.role;
        }
        ...
        if (widgetConfig) {
            var configStrArr = _.keys(widgetConfig);
            $.each(configStrArr, function(index, configStr) {
                configStr = configStr.replace(/'/g, '"');
                var _config = $.parseJSON(configStr) || {};
                config = _.defaults(_config, config);
            });
            delete dataset.widgetConfig;
        }
        //TODO datarole和widgetconfig优先级问题。
        config = _.defaults(dataset, config);
