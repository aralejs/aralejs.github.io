define(function(require, exports, module) {
    ...
    var Switchable = module.exports = Widget.extend({
        ...
        _parseRole: function() {
            ...
            if (role) {
                ...
                var triggersAndPanels = parseTriggerAndPanel($element,
                        {triggers: triggers, panels: panels}, role);

                console.log(triggersAndPanels)
                this.set('triggers', triggersAndPanels.triggers, SILENT);
                this.set('panels', triggersAndPanels.panels, SILENT);
                delete this.dataset.role;
            }

            for (var key in dataset) {
                if (dataset.hasOwnProperty(key)) {
                    this.set(key, _.keys(dataset[key])[0], SILENT);
                }
            }
        },
        ...
        _postCreate: function() {
            ...
            this._onRenderActiveIndex(willSwitch, -1);
        },
    ...
    function parseTriggerAndPanel($element, config, role) {
    ...
    Switchable.autoRender = function(config) {
        var role = config.dataset.role;
        var $element = $(config.element);

        if (role) {
            var triggersAndTriggers = parseTriggerAndPanel($element, config, role);
            config.triggers = triggersAndTriggers.triggers;
            config.panels = triggersAndTriggers.panels;
            delete config.dataset.role;
        }
