...
define(function(require, exports, module) {
    ...
    Carousel.autoRender = function(element, config, dataset) {
        var role = dataset.role;
        if (role) {
            if (role.nextButton) {
                config['prevButtonClass'] = role.nextButton;
            }
            if (role.prevButton) {
                config['nextButtonClass'] = role.prevButton;
            }
        }
        return Switchable.autoRender.call(Carousel, element, config, dataset);
    };
