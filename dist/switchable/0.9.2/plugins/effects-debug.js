...
define("#switchable/0.9.2/plugins/effects-debug", ["jquery","easing"], function(require, exports, module) {
...
//        EVENT_REMOVED = 'removed';
        ...
        install: function(host) {
            ...
            switch (effect) {
                ...
                case SCROLLY:
                    ...
                    if (!this.viewSize[0]) {
                        var m1 = 'switchable must specify viewSize';
                        var m2 = 'if there is no panels';
                        alert(m1 + m2);
                    }
                    break;
    ...
    Effects.methods = {
        // 最朴素的显示/隐藏效果
        none: function(toIndex, fromIndex) {
        ...
        // 淡隐淡现效果
        fade: function(toIndex, fromIndex) {
            var that = this;
                panelInfo = this._getFromToPanels(toIndex, fromIndex),
                fromPanels = panelInfo.fromPanels,
                toPanels = panelInfo.toPanels;
            ...
            if (fromEl && fromEl.length > 0) {
                ...
                this.anim = $(fromEl).animate({
                    opacity: 0
                }, this.duration, this.get('easing'), function() {
        ...
        // 水平/垂直滚动效果
        scroll: function(toIndex, fromIndex) {
            var that = this;
