    ...
    var Switchable = module.exports = Widget.extend({
        ...
        events: function() {
            ...
            if (this.get('triggerType') === 'hover') {
                ...
                hash['mouseleave .' + classTriggerInternal] = '_onMouseLeaveTrigger'; 
            }
            return hash;
        },
        ...
        _bind: function() {
            var that = this;
            this.on('change:activeIndex', this._switch, this);
            this.before('switchTo', function(toIndex, fromIndex) {
                this.trigger(EVENT_BEFORE_SWITCH, toIndex, fromIndex);
            });
            this.after('switchTo', function(toIndex, fromIndex) {
                that.trigger(EVENT_SWITCH, toIndex, fromIndex) 
            });
        ...
        _onMouseEnterTrigger: function(index) {
            ...
            this.switchTimer = utils.later(function() {
                //that.switchTo(index, undefined);
                that.set('activeIndex', index);
            }, that.get('delay') * 1000);
        },

        //鼠标移出 trigger 时触发的事件
        _onMouseLeaveTrigger: function(e) {
            e.stopPropagation();
        ...
        //通过监控activeIndex来驱动切换。
        _switch: function(toIndex, fromIndex) {
            var direction = toIndex > fromIndex ? FORWARD : BACKWARD;
            this.switchTo(toIndex, fromIndex, direction);
        },
        /**
         * 切换操作，对外 api
         ...
         */
        switchTo: function(toIndex, fromIndex, direction) {
            var triggers = this.triggers;
            // 再次避免重复触发
            if (!this._triggerIsValid(toIndex)) {
                return this;
            }
        ...
        _getFromToPanels: function(toIndex, fIndex) {
            var fromPanels,
                toPanels,
                step = this.get('step'),
                panels = this.panels;
        ...
        prev: function() {
            ...
            this.set('activeIndex', index, slient);
            this.switchTo(index, fromIndex , BACKWARD);
        },
        //切换到下一视图
        next: function() {
            // 循环
            ...
            this.set('activeIndex', index, slient);
            this.switchTo(index, fromIndex , FORWARD);
        },
        //插件安装
