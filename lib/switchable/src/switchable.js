define(function(require, exports, module) {
    ...
    // 内部默认的 className
    var PREFIX = 'ui-switchable-';
    var NAV_CLASS = PREFIX + 'nav';
    var CONTENT_CLASS = PREFIX + 'content';
    var TRIGGER_CLASS = PREFIX + 'trigger';
    var PANEL_CLASS = PREFIX + 'panel';
    ...
    var Switchable = Widget.extend({
        ...
        _parseRole: function() {
            ...
            if (triggers.length === 0 && (role.trigger || role.nav)) {
                triggers = element.find(role.trigger || role.nav).children();
            }

            if (panels.length === 0 && (role.panel || role.content)) {
                triggers = element.find(role.panel || role.content).children();
            }
        ...
        _initPanels: function() {
            var panels = this.panels || (this.panels = this.get('panels'));
            if (panels.length === 0) {
        ...
        _initTriggers: function() {
            var triggers = this.triggers || (this.triggers = this.get('triggers'));
        ...
        _onRenderActiveIndex: function(toIndex, fromIndex) {
            this.switchTo(toIndex, fromIndex);
        },
