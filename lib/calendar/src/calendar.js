...
define(function(require, exports, module) {
    ...
    //
    //     {now: now, dates: [...], startDay: 0}
    //
    function prepareData(now, startDay, available) {
        var returns = {now: now.clone()};
        ...
        if (delta != 0) {
            var previous = now.clone().add('months', -1);
        ...
        for (i = 1; i <= now.daysInMonth(); i++) {
            list.push({
                value: i,
                label: 'current',
                available: isAvailable(now.date(i), available)
            });
        }
        ...
        if (delta != 0) {
            var next = now.clone().add('months', 1);
            if (delta < 0) delta += 7;
            for (i = 1; i <= delta; i++) {
                list.push({
                    value: i,
