define(function(require, exports, module) {
    ...
    var CalendarModel = Base.extend({
        ...
        initialize: function(cal) {
            ...
            cal.i18n = cal.i18n || {};
            cal.i18n.today = "Today";

            var i18n = {};
            for (key in cal.i18n) {
                i18n[key] = {key: key}
                i18n[key].value = translate(cal.lang, cal.i18n[key])
            }
    ...
    function dateModel(current, available, startDay) {
        var items = [];
        var startDay = parseStartDay(startDay);
        ...
        var delta = measure.day() - startDay;
        if (delta != 0) {
        ...
        var formatedCurrent = current.format('YYYY-MM-DD');
        for (i = 1; i <= measure.daysInMonth(); i++) {
            var d = measure.date(i);
            if (d.format('YYYY-MM-DD') === formatedCurrent) {
                var status = 'focus';
            } else {
                var status = '';
            }
