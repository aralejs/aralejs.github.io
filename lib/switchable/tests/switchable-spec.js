define(function(require) {
    ...
    describe('Switchable', function() {
        ...
        var triggerActiveClass = 'ui-switchable-active';
        ...
        function testTriggerAndPanelActive(sw, active) {
            ...
            for (var i = 0, len = sw.length; i < len; i++) {
                ...
                panel = $(panels[i]);
                if (active == i) {
                    expect(trigger.attr('class')).toContain(triggerActiveClass);
                    expect(panel.css('display')).toBe('block');
                } else {
                    expect(trigger.attr('class')).not.toContain(triggerActiveClass);
